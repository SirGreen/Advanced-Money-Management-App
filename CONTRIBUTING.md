...
1.  **Sinh lại localization:** *NẾU* có chỉnh các file localization, cần chạy:
    ```bash
    flutter gen-l10n
    ```

2.  **Sinh lại các Hive entity:** *NẾU* có chỉnh các file entity, cần chạy:
    ```bash
...
### Cách tạo branch
* **Branch mới phải dựa trên branch master:** Trước khi tạo branch mới để chỉnh code và tạo pull request, cần phải chắc chắn branch đó được dựa trên branch `master` bằng cách chạy `git checkout master` *TRƯỚC* khi chạy lệnh `git branch xxxx`.
